...
import { DocumentType } from "../hooks/useDocumentsType";
import { Result } from "../hooks/useOrders";
...
export const generatePDF = ({
  ...
  doc.text(
    `: ${establishment}`,
    initialXSubheader + 2.5,
    initialYSubheader + 0.6
  );
  doc.text(
    `: ${typeOfCurrency.toUpperCase()}`,
    initialXSubheader + 2.5,
    initialYSubheader + 0.8
  );
  ...
  ordersLines?.map((order) => {
    const { result = "", observation = "", description = "" } = order || {};
    doc.text(result || "", 0.6, initialYBodyLeft + 0.2);
    doc.text("UNIDAD", 1.9, initialYBodyLeft + 0.2);
    doc.text(description || "", 3.3, initialYBodyLeft + 0.2);
    doc.text(observation || "", 3.3, initialYBodyLeft + 0.2);
  ...
  doc.text(
    `:       ${symbolOfCurrency} ${subtotal}`,
    7,
    initialYDescriptionRight
  );
  doc.text(
    `:       ${symbolOfCurrency} 0.00`,
    7,
    initialYDescriptionRight + 0.3
  );
  doc.text(
    `:       ${symbolOfCurrency} 0.00`,
    7,
    initialYDescriptionRight + 0.6
  );
  doc.text(
    `:       ${symbolOfCurrency} 0.00`,
    7,
    initialYDescriptionRight + 0.9
  );
  doc.text(
    `:       ${symbolOfCurrency} ${igv}`,
    7,
    initialYDescriptionRight + 1.2
  );
  doc.text(
    `:       ${symbolOfCurrency} 0.00`,
    7,
    initialYDescriptionRight + 1.5
  );
  doc.text(
    `:       ${symbolOfCurrency} 0.00`,
    7,
    initialYDescriptionRight + 1.8
  );
  doc.text(
    `:       ${symbolOfCurrency} 0.00`,
    7,
    initialYDescriptionRight + 2.1
  );
  doc.text(
    `:       ${symbolOfCurrency} 0.00`,
    7,
    initialYDescriptionRight + 2.4
  );
  doc.text(
    `:       ${symbolOfCurrency} ${total}`,
    7,
    initialYDescriptionRight + 2.7
  );

  // lineas de valor del contenido de la derecha
  doc.line(
    7.2,
    initialYDescriptionRight - 0.15,
    9.4,
    initialYDescriptionRight - 0.15,
    "F"
  );
