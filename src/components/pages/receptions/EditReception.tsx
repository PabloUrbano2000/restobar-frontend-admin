...
import { getReceptionById, updateReceptionById } from "../../../services";
import { showFailToast, showSuccessToast } from "../../../utils/toast";
import { Link } from "react-router-dom";
import { Reception } from "../../../types";

const EditReceptionPage = () => {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      try {
        ...
        const data = await updateReceptionById({
          id: reception?.id,
          code: values.code,
        ...
        if (data.status_code == 200) {
          showSuccessToast(
            data.message || "Recepción actualizada éxitosamente"
          );
  ...
  useEffect(() => {
    if (params.id) {
      const getProduct = async () => {
        setInProcess(true);
        const data = await getReceptionById(params?.id || "");
        if (data.status_code == 200) {
          const { reception: rec = {} } = data.data || {};
          setReception(rec || null);
          formik.setFieldValue("code", rec.code);
          formik.setFieldValue("number_table", rec.number_table);
          formik.setFieldValue("available", rec?.available?.toString());
          formik.setFieldValue("status", rec?.status?.toString());
